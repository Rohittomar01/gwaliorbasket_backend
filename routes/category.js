...
router.post('/add_company_category', upload.single('icon'), function (req, res, next) {
  ...
  pool.query("insert into categories(companyid, categoryname, description, icon, createdat, updatedat, createdby)values(?,?,?,?,?,?,?)", [req.body.companyid, req.body.category, req.body.description, req.file.originalname, req.body.createdat, req.body.updateat, req.body.createdby], function (error, result) {
    if (error) {
...
router.post('/edit_company_category', function (req, res, next) {
  pool.query("update categories set companyid=?, categoryname=?, description=?, updatedat=? , createdby=? where categoryid=?", [req.body.companyid, req.body.category, req.body.description, req.body.updateat, req.body.createdby, req.body.categoryid], function (error, result) {
    if (error) {
      console.log("xxxxx" + error)
      res.status(200).json({ status: false, message: 'Server error....' })
    }
